...
public class TransferToOthers extends JFrame implements ActionListener{
    ...
    public TransferToOthers(String mycountname, String countname, String wantsave) {
        ...
        jtf1.setText(list.get(0));  //姓名
        jtf2.setText(list.get(1));  //身份证号
        jtf3.setText(countname);	//卡号
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getActionCommand()=="确定")
        {
            String mynowmoney="";
            try {
                mynowmoney = new UserMessage().updatemoney(mycountname,-Integer.parseInt(wantsave));
            } catch (NumberFormatException e2) {
                e2.printStackTrace();
            } 
            if (Integer.parseInt(mynowmoney)>=0)
            {
                
                //对方账户存钱
                try {
                    new UserMessage().updatemoney(countname,Integer.parseInt(wantsave));
                } catch (NumberFormatException e1) {
                    e1.printStackTrace();
                } 
                JOptionPane.showMessageDialog(null, "转入成功，您的余额为："+mynowmoney,"温馨提示",JOptionPane.PLAIN_MESSAGE);
                dispose();
            }
            else 
            {
                JOptionPane.showMessageDialog(null, "余额不足请重新输入:","温馨提示",JOptionPane.WARNING_MESSAGE);
                dispose();
            }
        }
